import numpy as np
from typing import Mapping, Hashable, Union, Dict
...
class MergeMaskd(MapTransform):
    ...
    def __call__(self, data: Mapping[Hashable, Union[np.ndarray]]
                 ) -> Dict[Hashable, Union[np.ndarray]]:
        d = dict(data)
        mask_list = []
        for key in self.key_iterator(d):
        ...
        vert_range = np.intersect1d(np.unique(mask_list[0]), np.unique(mask_list[1]), assume_unique=True)
        vert_range = np.delete(vert_range, vert_range.argmin())  # background
        mask_list[1][~np.isin(mask_list[1], vert_range)] = 0
        merge_mask = np.where(mask_list[0] == 0, mask_list[1], 0) + mask_list[0]
