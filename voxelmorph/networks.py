...
class Unet(nn.Module):
    """A unet architecture. Layer features can be specified directly as a list of
    encoder and decoder features or as a single integer along with a number of unet
    levels. The default network features per layer (when no options are specified) are:
...
class VxmDense(LoadableModel):
    ...
    def __init__(self,
        ...
        self.integrate = layers.VecInt(down_shape,
                                       int_steps) if int_steps > 0 else None
    ...
    def forward(self, source, target, registration=False):
        ...
        if not registration:
            return (y_source, y_target,
                    preint_flow) if self.bidir else (y_source, preint_flow)
        else:
