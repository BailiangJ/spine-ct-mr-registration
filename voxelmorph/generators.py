...
def synthmorph(label_maps, in_shape, batch_size=1, same_subj=False, flip=True):
    """
    Generator for SynthMorph registration.

    Parameters:
        label_maps: List of label map path. Each as str.
    ...
    while True:
        ind = rand.integers(len(label_maps), size=2 * batch_size)
        x = [np.squeeze(py.utils.load_volfile(label_maps[i])) for i in ind]
        x = [np.where(vol != 0, 1, 0) for vol in x]
